...
<div class="col-12 col-md-12 col-lg-8">
    <form action=" {{ route('admin.clents.update', $clent->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="card">
            ...
            <div class="card-body">
                <div class="form-group">
                    <label>Name</label>
                    <input type="text" value="{{$clent->name}}" required class="form-control" name="name" @error('name')
                        is-invalid @enderror>
                    @error('name')
                ...
                <div class="form-group">
                    <label for="apartments_id">Apartments selected</label>
                    <select id="apartments_id" class="form-control" name="apartments_id" required>
                        <option value="" disabled>Choose an apartment</option>
                        @foreach ($apartments as $apartment)
                        <option value="{{ $apartment->id }}"
                            {{ $clent->apartments_id == $apartment->id ? 'selected' : '' }}>{{ $apartment->nom }}
                        </option>
                        @endforeach
                    </select>
                    @error('apartments_id')
...
$(document).ready(function() {
    $("#apartments_id").change(function() {
        ...
        $('#cars_id').html('');
        $.ajax({
            url: '{{ route('admin.getDistrictt') }}?id=' + regionCode,
            type: 'get',
            dataType: 'json',
